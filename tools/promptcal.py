...
def parse_args():
    ...
    # parser.add_argument('--checkpoint', default ="/gpfs/data/fs72306/leijx0408/segformer.b5.1024x1024.city.160k.pth", help='checkpoint file')
    parser.add_argument('--checkpoint', default ="/home/jonaslei/SVDP/segformer.b5.1024x1024.city.160k.pth", help='checkpoint file')
    ...
    parser.add_argument('--prompt_weight', type=float, default=0.)
    parser.add_argument('--consistency_weight', type=float, default=0., help='跨阶段一致性权重')
    parser.add_argument('--use_gt', action='store_true', help='是否在Prompt损失中使用GT')
    parser.add_argument('--align_weight', type=float, default=0.)
    parser.add_argument('--entropy_thresh', type=float, default=0.6,
...
def main():
    ...
    for i in range(num_itr):
        mean_mIoU = 0
        print(f"revisiting {i}")
        wandb.log({"revisiting_step": i})  # ✅ Ensure it gets logged
        wandb.run.log_code()  # ✅ Forces logs to sync immediately
        ...
        for dataset, data_loader in zip(datasets, data_loaders):
            ...
            if rank == 0:
                ...
                if args.eval:
                    ...
                    wandb.run.log_code()  # ✅ Force immediate sync
                    print(f'Logged mIoU: {mIoU}')  # ✅ Debugging
